...
describe("TypeSystem - Date", () => {
	...
	it("Encode", () => {
		...
		expect(Value.Encode<TDate, string>(schema, date)).toBe(date.toISOString());
	...
	it("Decode", () => {
		...
		expect(Value.Decode<TDate, Date>(schema, new Date())).toBeInstanceOf(Date);
		expect(Value.Decode<TDate, Date>(schema, "2021/1/1")).toBeInstanceOf(Date);
