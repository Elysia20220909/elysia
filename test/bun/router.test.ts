...
import { ELYSIA_REQUEST_ID, Elysia, t } from "../../src";
...
describe("Bun router", () => {
	it("works", async () => {
		...
		const response = await fetch(`http://localhost:${app.server!.port}`).then(
			(x) => x.json(),
		);
	...
	it("handle optional params", async () => {
		...
		const query = await Promise.all([
			fetch(`http://localhost:${app.server!.port}/id`).then((x) => x.json()),
			fetch(`http://localhost:${app.server!.port}/id/1`).then((x) => x.json()),
			fetch(`http://localhost:${app.server!.port}/id/1/saltyaom`).then((x) =>
				x.json(),
			),
		]);
	...
	it("trailing slash should be consistent with non-trailing slash", async () => {
		...
		expect(
			fetch(`http://localhost:${app.server?.port}/items/types`).then((x) =>
				x.text(),
			),
		...
		expect(
			fetch(`http://localhost:${app.server?.port}/items/types/a`).then((x) =>
				x.text(),
			),
