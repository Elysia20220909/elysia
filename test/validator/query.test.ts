...
import { type Context, Elysia, t, ValidationError } from "../../src";
...
describe("Query Validator", () => {
	...
	it("validate with hyphen in key", async () => {
		const app = new Elysia().get("/", ({ query }) => query["character-name"], {
			query: t.Object({
	...
	it("validate multiple", async () => {
		...
		const res = await app.handle(req("/?name=sucrose&job=alchemist&trait=dog"));
	...
	it("validate optional object", async () => {
		const app = new Elysia().get("/", ({ query }) => query?.name ?? "sucrose", {
			query: t.Object(
				{
	...
	it("create default string query", async () => {
		...
		const value = await app.handle(req("/?name=nagisa")).then((x) => x.json());
	...
	it("don't parse object automatically unless explicitly specified", async () => {
		...
		await app.handle(
			req(`/?pagination=${JSON.stringify({ pageIndex: 1, pageLimit: 10 })}`),
		);
	...
	it("slice non-ASCII querystring offset correctly", async () => {
		...
		expect(
			await app.handle(req("/?key1=ab&key1=cd&z=が")).then((x) => x.status),
		).toEqual(200);
		...
		expect(
			await app.handle(req("/?z=が&key1=ab&key1=cd")).then((x) => x.status),
		).toEqual(200);
	...
	it("handle array string correctly", async () => {
		...
		expect(response).toEqual([{}, { status: ["a"] }, { status: ["a", "b"] }]);
	});
	...
	it("handle reference query array", async () => {
		...
		const response = await app.handle(req("?ids=1,2,3")).then((x) => x.json());
	...
	it("handle numeric coercion in Union schema", async () => {
		...
		const response = await app.handle(req("/?page=5")).then((x) => x.json());
