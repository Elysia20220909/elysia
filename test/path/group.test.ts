...
describe("group", () => {
				...
				.group(
					...
					(app) =>
						app.group("/chapter", (app) =>
							app.get("/hello", ({ params: { courseId } }) => courseId),
						),
				),
		);
	...
	it("skip don't duplicate prefix on group with hooks", () => {
		...
		const b = new Elysia({ prefix: "/test" }).group("/id/:courseId", (app) =>
			app.get("/b", () => "A"),
		);
	...
	it("cast callback function schema to standaloneValidator", async () => {
		...
		const invalid = app.handle(req("/group/a/saltyaom")).then((x) => x.status);
