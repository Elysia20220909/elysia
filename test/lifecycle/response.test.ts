...
describe("On After Response", () => {
	...
	it("as global", async () => {
		...
		await Promise.all([app.handle(req("/inner")), app.handle(req("/outer"))]);
		// wait for next tick
		await Bun.sleep(1);
...
describe("On After Response Error", () => {
	...
	])("should execute onAfterResponse once during NotFoundError aot=$aot,\twithOnError=$withOnError", async ({
		aot,
		withOnError,
	}) => {
		let counter = 0;
	...
	])("should execute onAfterResponse when onError returns a value aot=$aot,\tonErrorReturnsValue=$onErrorReturnsValue", async ({
		aot,
		onErrorReturnsValue,
	}) => {
		let counter = 0;
