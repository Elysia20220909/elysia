...
describe("Parser", () => {
	...
	it("handle type with validator with custom parse", async () => {
		...
		const [correct, incorrect, custom] = await Promise.all([
			...
			app.handle(post("/json", { school: "Gehenna" })).then((x) => x.status),
			app
				.handle(
	...
	it("handle parser from plugin", async () => {
		const plugin = new Elysia().parser("custom", ({ contentType, request }) => {
			if (contentType === "application/x-elysia") return { name: "Eden" };
		});
