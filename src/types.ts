...
import type { AnyElysia, Elysia, InvertedStatusMap } from "./index";
...
import type { BunHTMLBundlelike } from "./universal/types";
...
> {
	...
	response: Schema["response"] extends FastAnySchema | string
		? {
				200: UnwrapSchema<Schema["response"], Definitions> extends infer A
					? A extends File
						? File | ElysiaFile
						: A
					: unknown;
...
> {
	body: UnwrapBodySchema<Schema["body"], Definitions>;
	headers: UnwrapSchema<Schema["headers"], Definitions> extends infer A extends
		Record<string, any>
		? A
		: undefined;
...
type MergeResponseStatus<A> = {
	[status in keyof UnionToIntersect<
		...
		A extends ElysiaCustomStatusResponse<any, any, infer Status>
			? { [A in Status]: 1 }
			: never
		// @ts-expect-error A is checked in key computation
...
type UnionMacroContext<A> = UnionToIntersect<{
	...
	// @ts-expect-error Allow recursive Macro.return without collapse into
	return: { _: A["return"] };
};
...
type InlineHandlerResponse<Route extends RouteSchema["response"]> = {
	[Status in keyof Route]: ElysiaCustomStatusResponse<
		// @ts-expect-error Status is always a number
		Status,
		Route[Status],
		Status
	>;
