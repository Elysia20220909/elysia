...
export const createStreamHandler =
	...
	async (
		...
		const isSSE =
			!skipFormat &&
			// @ts-expect-error First SSE result is wrapped with sse()
			(init?.value?.sse ??
				// @ts-expect-error ReadableStream is wrapped with sse()
				generator?.sse ??
...
export function mergeHeaders(
	...
	if (setHeaders instanceof Headers)
		...
		for (const key in setHeaders)
			if (key === "set-cookie") headers.append(key, setHeaders[key] as any);
			else if (!responseHeaders.has(key))
...
export const createResponseHandler = (handler: CreateHandlerParameter) => {
	...
	return (response: Response, set: Context["set"], request?: Request) => {
		...
		if (
			...
			(newResponse as Response).headers.get("transfer-encoding") === "chunked"
		)
			return handleStream(
