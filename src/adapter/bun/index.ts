...
export const BunAdapter: ElysiaAdapter = {
	...
	listen(app) {
		return (options, callback) => {
			...
			const createStaticRoute = <WithAsync extends boolean | undefined = false>(
				iterator: AnyElysia["router"]["response"],
				{ withAsync = false }: { withAsync?: WithAsync } = {},
			): true extends WithAsync
				? Promise<{
						[path: string]: Response | { [method: string]: Response };
					}>
				: {
						[path: string]: Response | { [method: string]: Response };
					} => {
				...
				if (withAsync) return Promise.all(ops).then(() => staticRoutes) as any;
	...
	ws(app, path, options) {
		...
		const validateMessage = messageValidator
			? messageValidator.provider === "standard"
				? (data: unknown) =>
						messageValidator.schema["~standard"].validate(data).issues
		...
		app.route(
			...
			path as any,
			async (context: any) => {
				const server = (context.server as (typeof app)["server"]) ?? app.server;
				...
				if (options.upgrade) {
					if (typeof options.upgrade === "function") {
						...
						Object.assign(set.headers, options.upgrade as Record<string, any>);
				}
				...
				const handleErrors = !hasCustomErrorHandlers
					...
					: async (ws: ServerWebSocket<any>, error: unknown) => {
							for (const handleError of errorHandlers) {
								let response = handleError(Object.assign(context, { error }));
				...
				if (
					server?.upgrade(context.request, {
						...
						data: {
							...
							message: async (ws: ServerWebSocket<any>, _message: any) => {
							...
							drain: async (ws: ServerWebSocket<any>) => {
								try {
									await handleResponse(
										ws,
										options.drain?.(new ElysiaWS(ws, context as any)),
									);
								} catch (error) {
									handleErrors(ws, error);
								}
