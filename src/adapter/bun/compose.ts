...
import type { InputSchema, InternalRoute } from "../../types";
import { ELYSIA_REQUEST_ID, isNotEmpty, randomId, redirect } from "../../utils";
...
export const createBunRouteHandler = (app: AnyElysia, route: InternalRoute) => {
	...
	const needsQuery =
		inference.query ||
		!!route.hooks.query ||
		!!(route.hooks.standaloneValidator as InputSchema[])?.find((x) => x.query);
