...
import { type ElysiaTypeCheck, getSchemaProperties } from "./schema";
...
const normalizeFormValue = (value: unknown[]) => {
	if (value.length === 1) {
		...
		if (typeof stringValue === "string") {
			...
			if (
				stringValue.charCodeAt(0) === 123 ||
				stringValue.charCodeAt(0) === 91
			) {
				try {
		...
		try {
								...
								if (typeof hook === "string")
									switch (hook) {
										...
										default: {
											...
											if (parser) {
												let temp = parser(context as any, contentType);
			...
			if (hooks.transform)
				for (let i = 0; i < hooks.transform.length; i++) {
					...
					if (hook.subType === "derive") Object.assign(context, response);
				}
			...
			if (validator) {
				...
				if (validator.createCookie?.()) {
					...
					if (validator.cookie!.Check(cookieValue) === false)
						...
						cookieValue = validator.cookie.Decode(cookieValue) as any;
				}
			...
			if (!hooks.afterHandle?.length) {
				...
				for (let i = 0; i < hooks.afterHandle.length; i++) {
					...
					if (isCustomStatuResponse) {
						set.status = status;
						response = (response as ElysiaCustomStatusResponse<any>).response;
					}
					...
					if (responseValidator?.Encode)
						context.response = response = responseValidator.Encode(response);

					if (responseValidator?.Clean)
						try {
							context.response = response = responseValidator.Clean(response);
						} catch {}
